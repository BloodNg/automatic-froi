...
def main():
    ...
    TreeN = [1,5,10,20,30,40,50,60]
    DepthM = [5,10,15,20,25,30]
    #TreeN = [1,50]
    #DepthM = [2,25]
        
    param = [[t,d] for t in TreeN for d in DepthM]
    print "mixed parameter set: ",param
    #total 423 feats
    ...
    for train, test in kf:    
        ...
        metric_set = np.zeros((48,10)) #average all fold and classes. mean and std.
        ...
        for i,p in enumerate(param):
            print 'Parameter: Tree_Num : %s ,Depth_Max : %s'%(p[0],p[1])
            metric_set[i][0] = p[0]
            metric_set[i][1] = p[1]
            ...
            metric_set[i][2] = Dice_mean
            metric_set[i][3] = Dice_std
            metric_set[i][4] = Precision_mean
            metric_set[i][5] = Precision_std
            metric_set[i][6] = Recall_mean
            metric_set[i][7] = Recall_std
            metric_set[i][8] = F1_mean
            metric_set[i][9] = F1_std

            filename = "param_select_score/outer_fold_%d_TreeN_%d_DepthM_%d.txt"%(k,p[0],p[1])
            np.savetxt(filename,metric_param,fmt='%1.8e',delimiter=',',newline='\n')
            print "evaluation score under a paramter pair for every class and fold have been saved in the %s"%filename
        ...
        filename = "param_select_score/eval_stat_fold_%s.txt"%k
        np.savetxt(filename,metric_set,fmt='%1.8e',delimiter=',',newline='\n')
        print "evaluation static score under a paramter pair have been saved in the %s"%filename
        #-----------------------------------------test part------------------------------------------
        ...
        print "feature importances:", clf.feature_importances_
        filename = "feat_importance_fold_%d.txt"%k
        ...
        testset_predict(sess_test,clf,mean,std,feat_mask,k,img)
        #outer level cv
        k+=1
    ...
    filename = "test_metric_sample_level.txt"
