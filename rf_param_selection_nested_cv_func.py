...
def main():
    ...
    print "Parameter set: ",TreeN,DepthM,LeafM,Fun
    #total 423 feats
    ...
    for train, test in kf:
        ...
        metric_set = np.zeros((2,9)) #average all fold and classes. mean and std.
        ...
        for i,F in enumerate(Fun):
            print 'Parameter: Tree_Num : 50 ,Depth_Max : 15 ,Leaf Sample Number Min : Fun : %s'%F
            metric_set[i][0] = i
            ...
            for train_m, cv_m in kfn:
                ...
                clf = RandomForestClassifier(bootstrap=True,criterion=F,max_depth=DepthM,
                      max_features=Feats,n_estimators=TreeN,min_samples_split=LeafM, n_jobs=100)
            ...
            filename = "param_select_score/outer_fold_%d_TreeN_50_DepthM_15_Fun_%d.txt"%(k,i)
            np.savetxt(filename,metric_param,fmt='%1.8e',delimiter=',',newline='\n')
            print "evaluation score under fun for every class and fold have been saved in the %s"%filename
        ...
        filename = "param_select_score/eval_stat_fold_%s_TreeN_50_DepthM_15_Fun.txt"%k
        np.savetxt(filename,metric_set,fmt='%1.8e',delimiter=',',newline='\n')
        print "evaluation static score under Fun have been saved in the %s"%filename
        k += 1
