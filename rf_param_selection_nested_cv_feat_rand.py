...
def main():
    ...
    DepthM = 15
    LeafM = 8
    Feats  = [10,20,50,100,150,200,250]
    ...
    for train, test in kf:
        ...
        metric_set = np.zeros((7,9)) #average all fold and classes. mean and std.
        ...
        for i,feat in enumerate(Feats):
            print 'Parameter: Tree_Num : 50 ,Depth_Max : 15 ,Feat Rand : %d'%feat
            metric_set[i][0] = feat
            ...
            for train_m, cv_m in kfn:
                ...
                clf = RandomForestClassifier(bootstrap=True,criterion='entropy',max_depth=DepthM,
                        max_features=feat,n_estimators=TreeN,min_samples_split=LeafM, n_jobs=100)
            ...
            filename = "param_select_score/outer_fold_%d_TreeN_50_DepthM_15_Feat_%d.txt"%(k,feat)
            np.savetxt(filename,metric_param,fmt='%1.8e',delimiter=',',newline='\n')
            print "evaluation score under feat rand for every class and fold have been saved in the %s"%filename
        ...
        filename = "param_select_score/eval_stat_fold_%s_TreeN_50_DepthM_15_Feat.txt"%k
